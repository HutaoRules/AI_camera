...
# === Configuration ===
rtmo_model_path = "/home/nvt/Workspaces/AI_Camera/camera/models/onnx/rtmo-l_16xb16-600e_body7-640x640-b37118ce_20231211/end2end.onnx"
stgcn_model_path = "/home/nvt/Workspaces/AI_Camera/camera/models/onnx/stgcnppbone100_2_17.onnx"
...
conf_values = pose_sequence[:, :, :, 2].flatten()
valid_conf = conf_values[conf_values > 0]
print(f"Number of keypoints with confidence > 0: {len(valid_conf)} out of {len(conf_values)}")
print(f"Average confidence of valid keypoints: {np.mean(valid_conf) if len(valid_conf) > 0 else 0}")
...
class_55_prob = probs[0][55] * 100 if 55 < probs.shape[1] else 0
print(f"\nClass 55 probability: {class_55_prob:.4f}%")
if 55 in top_indices:
    rank = list(top_indices).index(55) + 1
    print(f"Class 55 is ranked #{rank}")
else:
    print("Class 55 is not in the top results")
